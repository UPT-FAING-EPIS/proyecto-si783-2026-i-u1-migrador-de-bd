...
class CargadorDestino:
    ...
    def _quote_ident(ident: str, motor: str) -> str:
        ...
        if "mysql" in m or "mariadb" in m:
            return f"`{ident}`"
        if "sql server" in m or "microsoft sql server" in m:
            return f"[{ident}]"
        return f'"{ident}"'
    ...
    def _tipo_sql_destino(self, tipo_origen: str, motor: str) -> str:
        ...
        if any(x in t for x in self.INTEGER_TYPES):
            return "NUMBER" if "oracle" in m else "INTEGER"
    ...
    def _adaptar_sql_objeto(self, sql: str, motor: str) -> str:
        ...
        if "postgres" in m:
            ...
            s = re.sub(r"\[([^\]]+)\]", r"`\\1`", s)
            s = re.sub(r'"([^"]+)"', r"`\\1`", s)
        elif "sql server" in m or "microsoft sql server" in m:
            s = re.sub(r"`([^`]+)`", r"[\\1]", s)
            s = re.sub(r'"([^"]+)"', r"[\\1]", s)
    ...
    def generar_export(self, motor: str = None) -> tuple:
        ...
        if any(x in motor for x in ["mysql", "postgres", "oracle", "sql server", "mariadb"]):
